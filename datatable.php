...
{
$aColumns = array( 's.id','s.sname','s.balance','s.fees','c.career','b.branch','s.contact','s.joindate');
	
	/* Indexed column (used for fast and accurate table cardinality) */
	$sIndexColumn = "s.id";
	
	/* DB table to use */
	$sTable = " student as s,career as c, branch as b";
	...
	{
	$condArr[] = "c.id = '".mysqli_real_escape_string($conn,$_GET['career'])."'";
	...
	$sWhere = " WHERE c.id=s.career and b.id=s.branch and s.delete_status='0' and s.balance>0 ";
	...
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE c.id=s.career and b.id=s.branch and s.delete_status='0' and s.balance>0  ";
	...
	while ( $aRow = $rResult->fetch_assoc()  )
	{
		...
		$row = array(
					...
					html_entity_decode('<button class="btn btn-warning btn-xs" onclick="javascript:GetFeeForm('.$aRow['id'].', \''.$aRow['branch'].'\', \''.$aRow['career'].'\')"><i class="fa fa-usd "></i>  Tomar Pago </button>')
...
$aColumns = array( 's.id','s.sname','s.balance','s.fees','c.career','b.branch','s.contact','s.joindate');
	...
	$sWhere = " WHERE c.id=s.career and b.id=s.branch and s.delete_status='0'  ";
	if ( isset($_GET['sSearch'])&& $_GET['sSearch'] != "" )
	{
	...
	/* Total data set length */
	$sQuery = "SELECT COUNT(".$sIndexColumn.") as cc
		FROM   ".$sTable." WHERE c.id=s.career and b.id=s.branch and s.delete_status='0'   ";
	$rResultTotal = $conn->query( $sQuery ) or die(mysqli_error($conn));
	$aResultTotal = $rResultTotal->fetch_assoc();
	$iTotal = $aResultTotal['cc'];
