...
if (isset($_POST['login'])) {
    ...
    if ($q->num_rows == 1) {
        $res = $q->fetch_assoc();
        if ($res['role_name'] == 'ROLE_ADMIN' || $res['role_name'] == 'admin') {
            $_SESSION['rainbow_username'] = $res['username'];
            $_SESSION['rainbow_uid'] = $res['id'];
            $_SESSION['rainbow_name'] = $res['name'];
            $_SESSION['rainbow_role'] = 'admin';
        } elseif ($res['role_name'] == 'ROLE_SECRETARIA' || $res['role_name'] == 'secretaria') {
            $_SESSION['rainbow_username'] = $res['username'];
            $_SESSION['rainbow_uid'] = $res['id'];
            $_SESSION['rainbow_name'] = $res['name'];
            $_SESSION['rainbow_role'] = 'secretaria';
        }
